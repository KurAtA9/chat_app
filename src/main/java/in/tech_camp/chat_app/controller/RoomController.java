...
public class RoomController {
  ...
  @PostMapping("/rooms")
  public String createRoom(@ModelAttribute("RoomForm") @Validated() RoomForm roomForm, @AuthenticationPrincipal CustomUserDetail currentUser, Model model){
    ...
    for (Integer userId : memberIds) {
      ...
      try {
        ...
        System.out.println("エラー：" + e);
        List<UserEntity> users = userRepository.findAllExcept(currentUser.getId());//さらのページにリダイレクトするので、処理中に取得しとかんとあかん
        model.addAttribute("users", users);
        model.addAttribute("roomForm", new RoomForm());
        return "rooms/new";
      }
